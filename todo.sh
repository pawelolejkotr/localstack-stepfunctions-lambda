...
aws --endpoint-url=http://localhost:4566 stepfunctions create-state-machine --name MyLocalStateMachine --definition file://stepCallParamFunc.json --role-arn arn:aws:iam::000000000000:role/MyStepFunctionsRole --type STANDARD

aws --endpoint-url=http://localhost:4566 stepfunctions create-state-machine `
    --name ParallelRandomGenerator `
    --definition file://state_machine_map_definition.json `
